#!/usr/bin/env python
"""
Test script for the element analysis functionality in the natal chart API.
This script sends a request to the API with test birth data and prints the element analysis results.
"""
...
def test_element_analysis():
    ...
    and printing the element analysis results.
    """
    # API endpoint
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with open("element_analysis_response.json", "w") as f:
                json.dump(result, f, indent=2)
                
            print("\n=== Element Analysis Test Results ===")
            ...
            # Also check if elementsPower is present (the original implementation)
            elements_power = result.get("elementsPower", {})
            if elements_power:
                print("\n=== Original Elements Power ===")
                for element, percentage in elements_power.items():
                    print(f"{element}: {percentage:.1f}%")
